...
for program in all_programs['programs']:
	...
	for work in program['works']:
		...
		# here is the ID, assuming this is a unqiue id for this work
		# print(work['ID'])

		work_id = work['ID']

		# lets save this work in our all works as a dict, so it has some meetadata
		...
		if work_id not in all_works:
			...
			all_works[work_id] = { "title" : title, "composer" : composerName, "dates": [], "decades" : []  }
		...
		all_works[work_id]['dates'].append(program["season"])


# we can loop through them now, by work id
for work_id in all_works:

	#print(all_works[work_id])

	# and here are all the dates
	for date in all_works[work_id]['dates']:
		...
		all_works[work_id]['decades'].append(decade)
...
fields = ['work_id','title','composer','1840','1850','1860','1870','1880','1890','1900','1910','1920','1930','1940','1950','1960','1970','1980','1990','2000','2010','2020']

with open('work_with_decade_count.csv','w') as outfile:
	...
	for work_id in all_works:
		...
		work = {
			"work_id": work_id,
			"title": all_works[work_id]['title'],
			"composer": all_works[work_id]['composer'],
			# and now all the years we just count the number of times that decade appears in our list of decades it was played
			'1840': all_works[work_id]['decades'].count('1840'),
			'1850': all_works[work_id]['decades'].count('1850'),
			'1860': all_works[work_id]['decades'].count('1860'),
			'1870': all_works[work_id]['decades'].count('1870'),
			'1880': all_works[work_id]['decades'].count('1880'),
			'1890': all_works[work_id]['decades'].count('1890'),
			'1900': all_works[work_id]['decades'].count('1900'),
			'1910': all_works[work_id]['decades'].count('1910'),
			'1920': all_works[work_id]['decades'].count('1920'),
			'1930': all_works[work_id]['decades'].count('1930'),
			'1940': all_works[work_id]['decades'].count('1940'),
			'1950': all_works[work_id]['decades'].count('1950'),
			'1960': all_works[work_id]['decades'].count('1960'),
			'1970': all_works[work_id]['decades'].count('1970'),
			'1980': all_works[work_id]['decades'].count('1980'),
			'1990': all_works[work_id]['decades'].count('1990'),
			'2000': all_works[work_id]['decades'].count('2000'),
			'2010': all_works[work_id]['decades'].count('2010'),
			'2020': all_works[work_id]['decades'].count('2020')
		}
